...
## 用户反馈修订：固定首页（2026-09-16）

用户指出内容过多与滚动不符合预期，并要求移除顶部标题。首页已改为不含 ScrollView 的固定视口，删除易链/YILIAN CONNECT 顶栏、重复说明和堆叠卡片。中部圆环按剩余空间缩小；说明、指标与错误根据状态显示，完整原因/诊断放在可点击弹层，紧凑高度将指标收至详情。活动与登录仍可在内容较多时滚动，但不显示滚动条。

验证：重新构建、35 项 JVM 测试与 lint；设备端视觉检查仍待真机。
