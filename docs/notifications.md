...
通知设置提供审批邮件主题、HTML / 纯文本正文和示例预览。新配置默认使用带品牌页头、授权明细和到期提醒的 HTML 模板；已有纯文本模板保持原格式，可通过“使用默认 HTML 模板”切换并保存。支持 `{{username}}`、`{{applicant_email}}`、`{{user_groups}}`、`{{expires_at}}`、`{{instance_code}}`，变量内允许首尾空格。
...
HTML 邮件以 `multipart/alternative` 发送，同时附带系统生成的纯文本授权摘要。正文变量自动进行 HTML 实体转义，请将变量放在文本位置，不要用于脚本、样式或 URL。模板建议使用表格布局和行内样式，以兼容邮件客户端。后台预览在无脚本权限的隔离 iframe 中显示，并阻止外部资源请求；邮件客户端实际效果可能略有不同。格式字段 `html` 缺省为 `false`，兼容历史保存的模板。
