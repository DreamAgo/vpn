...
| 能力 | Android 行为 |
| --- | --- |
| 密码与飞书登录 | 独立登录页提供两种方式；飞书未配置时仍可密码登录，优先通过 AppLink 唤起飞书 App，无法唤起时使用浏览器；授权地址固定 `accounts.feishu.cn` HTTPS 主机 |
...
- 飞书 App 授权及未安装时浏览器兜底、授权后自动返回易链及被拦截时手动返回、取消/超时、旋转重建、真实账号显示。
...
VPN 前台通知使用易链盾牌 Y 的单色图标，系统自带的 VPN 钥匙标识仍由 Android 控制。通知显示相邻采样的真实上传/下载速率，按 B/s、KB/s、MB/s 自动换算；首页和详情保留累计流量语义，按 B、KB、MB 换算，每 1024 进一档。重连创建新的采样器，计数回退时速率归零，避免显示负速率或沿用上一连接。

本轮构建与 lint 通过，39 项 JVM 测试通过（含单位边界、采样间隔、空闲与计数重置）。通知图标资源已核对打包；尚未连接设备进行状态栏真机检查。

## Android 飞书 App 授权

按[飞书官方 AppLink 接入说明](https://open.feishu.cn/document/uAjLw4CM/uYjL24iN/mobile-app/integrate-via-app-link)，将服务端生成的 OAuth URL 编码进 `https://applink.feishu.cn/client/web_url/open?mode=window&url=...`，用限定 `com.ss.android.lark` 包名的 Android Intent 打开，避免被默认浏览器截获。没有可处理该链接的飞书 Activity 时才打开原始浏览器授权页；两者都不可用时提示安装飞书。
