...
客户端 DNS 仅提供“关闭”和“全局”。启用全局后，将 VPN 网关设为系统默认 DNS；这不改变 VPN 数据流量路由，也不接管应用自行配置的 DoH，不覆盖其他 VPN 或系统已有的更具体 DNS 策略。服务端按域名选择上游、静态 A 记录仍可独立配置，不需要客户端域名列表。客户端断开时仅恢复本产品设置，应用失败会回滚。客户端先启动隧道转发，通过 VPN 地址向网关发起根 NS 查询，验证成功才启用系统 DNS。运行期间每 5 秒检测；为允许最多 8 个上游依次故障切换，单次探测最多等待 18 秒。探测失败后恢复系统原有 DNS，恢复后再次接管。因此该模式优先保持联网，不是严格的 DNS 防泄漏模式；降级期间私有域名也可能由系统原有 DNS 处理。

Windows 启动及无活动隧道的连接前，会在访问服务端域名之前清理本产品遗留规则。新 NRPT 规则由独立 PowerShell 租约进程持有：正常断开或客户端进程退出导致管道关闭时，租约清理自己的规则；规则使用易失性注册表键，在完整重启后不保留。启动清理兼容旧版本的持久规则，通过租约进程 PID 和启动时间避开其他仍活动的实例。无需删除整个 `DnsPolicyConfig`。快速启动/休眠不等于完整重启；真实 Windows 上的快速启动、断电及杀进程恢复仍需按 [DNS 恢复验证清单](dns-recovery-validation.md) 验证。
...
内置 DNS 只绑定 VPN 网关地址的 UDP/TCP 53，不应在 Docker `ports` 或主机防火墙中发布公网 53。它只接受 VPN 子网来源，静态 A 记录优先，转发规则按最长域名后缀选择上游，其余使用默认上游，并支持上游故障切换（超时、SERVFAIL、REFUSED 等错误尝试下一上游；NXDOMAIN 为有效结果）、UDP 截断后的 TCP 回退和有界缓存。静态名称的 AAAA 等非 A 查询返回空答案，不再转发到公网。TTL 为 0 及没有 SOA 的负响应不缓存；根 NS 健康查询绕过缓存，以便发现默认上游中断。DNS 监听失败会重试，不再导致 HTTP 控制面退出。Linux 客户端需要 systemd-resolved 的 `resolvectl`；客户端不会修改 `/etc/resolv.conf` 或物理网卡 DNS。
...
“客户端版本 → 同步设置 → 最低客户端版本”设置允许连接的最低正式版本（如 `0.1.32`），留空关闭限制。适用于所有上报版本的桌面端、CLI 和节点客户端。按语义版本比较，低版本、无版本、无效版本的新注册会被拒绝；离线节点通过心跳恢复及导出旧节点配置也会校验。已有在线节点的心跳不受影响，不主动断开现有连接。登录、下载更新不受限制。设置保存后生效，重启后保留；API 字段为 `minimum_client_version`，省略保留原值、空字符串关闭。
