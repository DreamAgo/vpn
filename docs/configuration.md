...
客户端支持[自动内网直连](lan-direct-routing.md)：对配置范围内新建的普通 IPv4 TCP 连接，先经物理接口尝试真实业务端口，成功后复用同一条连接直连，失败再走 VPN。包含经默认网关互通的不同网段，首次长连接也适用，不需要等待同一 IP 的其他 VPN 会话结束。网段仍由现有后台配置下发，无需新增客户端配置；UDP、ICMP、IPv6 等保持原 VPN 转发。
