...
export function NetworkSettingsPage() {
  ...
  const restarting = useIsMutating({ mutationKey: ['restart-server'] }) > 0;
  ...
  return <div>
    <div className="page-heading">
      ...
      <Button type="primary" icon={<SaveOutlined />} disabled={restarting || !data || isError} loading={mutation.isPending} onClick={() => void save()}>保存配置</Button>
    </div>
        ...
        <Card title="按网络排除 VPN 路由" style={{ marginTop: 16 }}>
          ...
          <Form.List name="localRouteBypass" rules={[{
            ...
            {(fields, { add, remove }, { errors }) => <Space direction="vertical" size={12} style={{ width: '100%' }}>
              ...
              <Button type="dashed" icon={<PlusOutlined />} disabled={restarting || !data || isError || mutation.isPending || fields.length >= MAX_BYPASS_RULES} onClick={() => add({ localSubnets: [], excludedRoutes: [] })}>添加排除规则（{fields.length}/{MAX_BYPASS_RULES}）</Button>
            </Space>}
          </Form.List>
        </Card>
