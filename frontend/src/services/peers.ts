...
import type { AdminPeerQuery, AdminPeerView, Page, PeerEventQuery, PeerEventView } from '@/types/api';
...
export const peersApi = {
  ...
  /** 节点变更记录（OS / IP / Endpoint 变化；节点健康监控）。 */
  async listPeerEvents(query: PeerEventQuery = {}): Promise<PeerEventView[]> {
    const res = await http.get<PeerEventView[]>('/admin/peer-events', { params: query });
    return res.data;
  },
