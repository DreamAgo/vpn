...
impl UserService {
    ...
    pub async fn create_user(
        ...
        let row = self
            .user_repo
            .insert(&user_id, username, email, &hash, "user", true)
            .await?;
...
mod tests {
    ...
    async fn create_user_generates_password_when_omitted() {
        ...
        let resp = svc.create_user("alice", "alice@e.com", None).await.unwrap();
    ...
    async fn create_user_uses_provided_password() {
        let svc = service(setup_pool().await);
        let resp = svc
            .create_user("bob", "bob@e.com", Some("Sup3rSecret"))
            .await
            .unwrap();
    ...
    async fn create_user_rejects_weak_provided_password() {
        let svc = service(setup_pool().await);
        let err = svc
            .create_user("bob", "bob@e.com", Some("short"))
            .await
            .unwrap_err();
    ...
    async fn create_user_duplicate_returns_duplicate_resource() {
        let svc = service(setup_pool().await);
        svc.create_user("alice", "alice@e.com", None).await.unwrap();
        let err = svc
            .create_user("alice", "other@e.com", None)
            .await
            .unwrap_err();
    ...
    async fn disable_user_revokes_sessions() {
        ...
        let created = svc.create_user("alice", "alice@e.com", None).await.unwrap();
    ...
    async fn list_users_applies_defaults_and_search() {
        let svc = service(setup_pool().await);
        svc.create_user("alice", "alice@e.com", None).await.unwrap();
        svc.create_user("bob", "bob@e.com", None).await.unwrap();
